...
def is_same_pt(ptA, ptB):
    return (
        are_floats_equal(ptA[0], ptB[0])
        and are_floats_equal(ptA[1], ptB[1])
        and are_floats_equal(ptA[2], ptB[2])
    )
...
def gcode_move(current_pos, next_pos, feed_rate=None, should_extrude=False):
    ...
    x_from = current_pos[0]
    y_from = current_pos[1]
    z_from = current_pos[2]
    x_to = next_pos[0]
    y_to = next_pos[1]
    z_to = next_pos[2]
    ...
    if should_extrude:
        A_extrusion_cross_section = (extrusion_width - layer_height) * layer_height + math.pi * pow(layer_height / 2,2)
        d_euclidean = math.sqrt(pow(x_from-x_to,2) + pow(y_from-y_to,2) + pow(z_from-z_to,2))
        V_out = A_extrusion_cross_section * d_euclidean
        L = V_out / (math.pi * pow(filament_diameter / 2,2))
        e_value = float_to_string(L, precision=E_VALUE_PRECISION)
...
def generate_gcode():
    ...
    for i in range(0, len(lines)):
        ...
        if not are_floats_equal(current_position[2], line_start_position.Z):
            next_pos = list(current_position)
            next_pos[2] = line_start_position.Z
            move_z_command = gcode_move(current_position, next_pos, feed_rate=layer_change_feed_rate)
            all_move_commands.append(move_z_command)
        ...
        if not is_same_pt(current_position, line_start_position):
            ...
            move_to_line_start_command = gcode_move(
                current_position, line_start_position, feed_rate=travel_feed_rate
            )
            # Append command
        ...
        extrude_command = gcode_move(
            current_position, line_end_position, extrusion_feed_rate, True
        )

        # [TODO]: Append the move command across the line segment
        all_move_commands.append(extrude_command)
